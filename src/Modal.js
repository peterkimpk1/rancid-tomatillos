...
export default function Modal({ singleMovieDetails }) {
  ...
  useEffect(() => {
    getMovieTrailer(movie_id).then((data) => setTrailerURL(`https://www.youtube.com/embed/${data.videos[0].key}`))
  }, [])
  const ref = useRef()
  ...
  const allGenres = genres.map((genre) => {
    let uniqueID = uuidv4()
    return (
      <div key={uniqueID} className='genre'>
        {genre}
      </div>
    )
  })
  return (
    <dialog data-modal className='modal' open>
      <div className='image-button-wrapper'>
        ...
        <Link to='/'>
          <button className='button-close' formMethod='dialog' autoFocus>
            Return to Main
          </button>
        </Link>
      </div>
      <div className='title-genre-wrapper'>
        <h2>{title}</h2>
        <div className='genres'>{allGenres}</div>
      </div>
          ...
          <div className='movie-info-wrapper'>
            <div className='movie-card-info' id='movie-trailer'>
              ...
              <dialog ref={ref} onCancel={closeModal} className='trailer-modal'>
                <button className='trailer-close-btn' onClick={closeModal}>
                  Close
                </button>
                <iframe
                  src={trailerURL}
