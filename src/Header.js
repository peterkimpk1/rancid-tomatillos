      ...
      <nav>
        <div className='title-filter-wrapper'>
          ...
          <div className='search-wrapper'>
            Search By Title:
            <input placeholder='Search' className='search' value={searchValue} onChange={(e) => setSearchValue(e.target.value)}></input>
          </div>
          <div className='genre-wrapper'>
            <label htmlFor='genres'>Search By Genre:</label>
            <select id='genres' name='genres' value={selectionValue} onChange={(e) => setSelectionValue(e.target.value)}>
              <option value=''>View All</option>
              {selectionGenres}
            </select>
          </div>
        </div>
        {searchValue && selectionValue && (
          <p className='filter-message'>
            Filtering by:&nbsp;&nbsp;[Genre: <strong>{selectionValue}</strong> &nbsp; Title: <strong>{searchValue}</strong>]
          </p>
        )}
        {searchValue && !selectionValue && (
          <p className='filter-message'>
            Filtering by:&nbsp;&nbsp;[Title: <strong>{searchValue}</strong>]
          </p>
        )}
        {!searchValue && selectionValue && (
          <p className='filter-message'>
            Filtering by:&nbsp;&nbsp;[Genre: <strong>{selectionValue}</strong>]
          </p>
        )}
      </nav>
    </>
